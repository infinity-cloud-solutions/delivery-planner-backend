...
# Own's modules
from data_access.geolocation_handler import Geolocation
from data_mapper.order_mapper import OrderHelper
from models.order import HIBerryOrder
from utils.doorman import DoormanUtil
...
def lambda_handler(event: Dict(str, Any), context: LambdaContext) -> Dict(str, Any):
    ...
    try:
        ...
        logger.info(
            f"Processing order for: {new_order_data.get('client_name')} at {new_order_data.get('delivery_address')}"
        )
        location_service = Geolocation()
        lat_and_long = location_service.get_lat_and_long_from_street_address(
            str_address=new_order_data.get("delivery_address")
        )
        ...
        return doorman.build_response(payload={"Result": "Record was created"}, status_code=201)
        ...
        return doorman.build_response(payload={"Result": error_details}, status_code=400)

    except AuthError:
        logger.error(f"user {username} was not auth to create a new order")
        return doorman.build_response(payload={"Result": error_details}, status_code=500)

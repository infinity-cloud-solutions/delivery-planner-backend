...
class DeliveryProcessor:
    ...
    def process_records_for_driver(self, driver_number, orders_for_today, dao):
        ...
        afternoon_records = processor.select_orders_by_delivery_range_time(
            order_records=orders_for_today, delivery_time_to_match="1 PM - 5 PM", driver_to_match=driver_number
        )
        if len(afternoon_records) > 0:
            logger.info(f"Records to schedule for 1 PM - 5 PM delivery for Driver {driver_number}: {len(afternoon_records)}")
